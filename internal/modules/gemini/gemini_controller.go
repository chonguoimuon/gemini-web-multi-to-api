...
func (h *GeminiController) HandleV1BetaGenerateContent(c fiber.Ctx) error {
	...
	if err != nil {
		...
		} else if errors.Is(err, providers.ErrAllAccountsExhausted) {
			status = fiber.StatusServiceUnavailable
			errorType = "service_unavailable"
		}
		
		errResp := common.ErrorToResponse(err, errorType)
	...
	c.RequestCtx().SetBodyStreamWriter(func(w *bufio.Writer) {
		...
		if err != nil {
			...
			if errors.Is(err, providers.ErrAllAccountsExhausted) {
				errResponse = common.ErrorToResponse(err, "service_unavailable")
			} else {
