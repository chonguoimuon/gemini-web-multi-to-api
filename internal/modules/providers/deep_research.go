...
func (c *Client) DeepResearch(ctx context.Context, query string, opts ...DeepResearchOption) (*DeepResearchResult, error) {
	...
	for i, q := range plan.SubQuestions {
		...
		// Pacing: Wait 1.5 seconds between sub-questions to avoid Google anti-bot
		select {
...
func (c *Client) DeepResearchStream(ctx context.Context, query string, cb ProgressCallback, opts ...DeepResearchOption) error {
	...
	c.log.Info("==================================================")
	c.log.Info("🔬 Deep research stream starting", zap.String("query", query), zap.String("model", model))
	...
	c.log.Info("✅ Deep research stream completed", zap.String("query", query))
	c.log.Info("==================================================")
	return nil
}
