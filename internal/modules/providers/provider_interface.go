...
type ChatSession interface {
	...
	// GetAccountID returns the account ID associated with this session
	GetAccountID() string
	
	// GetMetadata returns session metadata for persistence
	GetMetadata() *SessionMetadata
	
	// Clear clears the conversation history locally
	Clear()

	// Delete deletes the conversation memory on the provider side
	Delete() error
}
